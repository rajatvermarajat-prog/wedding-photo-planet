...
export function ExpenseManagement({ projects, freelancers, currentUser }: { projects: Project[]; freelancers: FreelancerOption[]; currentUser: TeamMember }) {
  ...
  const canApprove = ['Owner', 'Studio Manager', 'Manager', 'Account Manager'].includes(currentUser.role);
  ...
  const openEdit = (expense: Expense) => { setDraft({ ...expense }); setSelected(expense); setModal('form'); };
  const saveExpense = (event: React.FormEvent) => {
    ...
    if (selected) {
      ...
      setExpenses((items) => [{ ...draft, id, paidAmount, paymentStatus, approvalStatus: canApprove ? 'Approved' : 'Submitted', createdAt: timestamp, updatedAt: timestamp }, ...items]);
  ...
  const remove = () => { if (!selected) return; setExpenses((items) => items.filter((item) => item.id !== selected.id)); setModal(null); setSelected(null); toast.showToast('Expense deleted.'); };
  ...
  return <div className="mx-auto w-full max-w-[1600px] space-y-5 pb-12">
    <header className="overflow-hidden rounded-3xl border border-[#8f7368] bg-[linear-gradient(120deg,#74475a,#3e2b31)] px-5 py-6 text-white shadow-xl sm:px-8">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2"><button onClick={() => exportPdf()} className="flex items-center gap-2 rounded-xl border border-white/25 bg-white/10 px-4 py-2.5 text-xs font-bold hover:bg-white/20"><Download className="h-4 w-4"/> Export PDF</button><button onClick={() => openAdd()} className="flex items-center gap-2 rounded-xl bg-white px-4 py-2.5 text-xs font-black text-[#5a2f3e] shadow-lg"><Plus className="h-4 w-4"/> Add Expense</button></div>
      </div>
    </header>
    ...
    {section === 'recurring' ? <RecurringView items={recurring} /> : section === 'reports' ? <ReportsView expenses={expenses} onExport={exportPdf} /> : <>
      ...
      {section !== 'overview' && <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div><h2 className="text-lg font-black text-slate-900">{sections.find((item) => item.id === section)?.label}</h2><p className="text-xs text-slate-500">{visibleExpenses.length} records · {money(sum(visibleExpenses))} total</p></div><Filters search={search} setSearch={setSearch} category={categoryFilter} setCategory={setCategoryFilter} status={statusFilter} setStatus={setStatusFilter} /><button onClick={() => openAdd(categoryMap[section]?.[0])} className="flex shrink-0 items-center justify-center gap-2 rounded-xl bg-rose-700 px-4 py-2.5 text-xs font-bold text-white"><Plus className="h-4 w-4"/> Add Expense</button></div></div>}
      <ExpenseTable expenses={section === 'overview' ? expenses.slice(0, 6) : visibleExpenses} projects={projects} onView={(expense) => { setSelected(expense); setModal('detail'); }} onEdit={openEdit} />
    </>}

    {modal === 'form' && <ExpenseForm draft={draft} setDraft={setDraft} projects={projects} freelancers={freelancers} editing={!!selected} onClose={() => setModal(null)} onSubmit={saveExpense} />}
    {modal === 'detail' && selected && <ExpenseDetail expense={selected} project={projects.find((p) => p.id === selected.projectId)} canApprove={canApprove} onClose={() => setModal(null)} onEdit={() => openEdit(selected)} onDelete={() => setModal('delete')} onApproval={updateApproval} onMarkPaid={markPaid} />}
    {modal === 'delete' && selected && <div className="fixed inset-0 z-[70] grid place-items-center bg-slate-950/55 p-4 backdrop-blur-sm"><div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl"><div className="mb-4 grid h-12 w-12 place-items-center rounded-2xl bg-red-100 text-red-600"><Trash2/></div><h3 className="text-lg font-black">Delete {selected.id}?</h3><p className="mt-2 text-sm text-slate-500">This expense and its payment history will be removed.</p><div className="mt-6 flex justify-end gap-2"><button onClick={() => setModal('detail')} className="rounded-xl border px-4 py-2 text-sm font-bold">Cancel</button><button onClick={remove} className="rounded-xl bg-red-600 px-4 py-2 text-sm font-bold text-white">Delete Expense</button></div></div></div>}
  </div>;
}
