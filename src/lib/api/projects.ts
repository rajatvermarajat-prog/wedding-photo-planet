...
export interface Project { id:string; projectNumber:string; name:string; type:BackendProjectType; status:BackendProjectStatus; weddingDate:string|null; deliveryDueDate:string|null; venueName:string|null; venueAddress:string|null; venueCity:string|null; totalQuotation:string; customServiceType:string|null; otherClientDetails:string|null; notes:string|null; client:ProjectClient; tasks?:BackendTask[]; shoots?:BackendShoot[]; createdAt:string; updatedAt:string; _count?:{events:number;shoots:number;tasks:number;deliveries:number}; }
